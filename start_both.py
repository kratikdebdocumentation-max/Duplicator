#!/usr/bin/env python3
"""
Duplicator Trading Bot - Unified Launcher
Starts both Telegram bot and Web server interfaces simultaneously
"""
...
class UnifiedTradingApp:
    """Unified application that runs both Telegram and Web interfaces"""
    
    def __init__(self):
        self.logger = get_logger('unified_app')
    ...
    def initialize(self) -> bool:
        ...
        try:
            self.logger.info("Initializing Unified Duplicator Trading Bot...")
    ...
    def start_web_server(self, host: str = "127.0.0.1", port: int = 8000) -> bool:
    ...
    async def run(self, web_host: str = "127.0.0.1", web_port: int = 8000) -> None:
        """Run the unified application"""
        try:
            self.logger.info("Starting Unified Duplicator Trading Bot...")
            ...
            # Start web server
            if not self.start_web_server(web_host, web_port):
                self.logger.error("Failed to start web server")
                return
            
            self.is_running = True
            self.logger.info("Unified Duplicator Trading Bot started successfully!")
    ...
    async def _send_startup_notification(self) -> None:
        ...
        try:
            if self.telegram_bot:
                ...
                message = (
                    f"🚀 *Unified Duplicator Bot Started*\n\n"
    ...
    async def stop(self) -> None:
        ...
        try:
            self.logger.info("Stopping Unified Duplicator Trading Bot...")
            self.is_running = False
            ...
            self.logger.info("Unified Duplicator Trading Bot stopped")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Unified Duplicator Trading Bot')
    ...
    asyncio.run(app.run(args.web_host, args.web_port))
